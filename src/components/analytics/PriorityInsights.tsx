      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Top recommendations to improve your curriculum
          </p>
        </div>
        ...
        {insights.map((insight, idx) => {
          ...
          const bgColor =
            insight.type === "critical"
              ? "from-red-50 to-orange-50 dark:from-red-500/10 dark:to-orange-500/10"
              : insight.type === "warning"
              ? "from-yellow-50 to-amber-50 dark:from-yellow-500/10 dark:to-amber-500/10"
          ...
          return (
            <Card
              key={idx}
              className={`border ${borderColor} bg-linear-to-r ${bgColor}`}
            >
              <CardContent className="p-5">
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center shrink-0 shadow-sm`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <span
                        className={`px-2 py-1 text-[10px] font-bold rounded uppercase whitespace-nowrap ${
                          insight.impact === "High"
