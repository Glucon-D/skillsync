...
export default function AssessmentPage() {
  ...
  useEffect(() => {
    ...
    if (!isCompleted) {
      loadProgressFromLocalDB();
      console.log("[Assessment] 📂 Loaded progress from LocalDB");
    }
  }, [isCompleted, loadProgressFromLocalDB]);
  ...
  const handleAnswer = () => {
    if (selectedAnswer !== null && currentQ) {
      ...
      saveProgressToLocalDB();
      console.log("[Assessment] 💾 Progress saved to LocalDB");
      ...
      if (currentQuestion < quizQuestions.length - 1) {
        ...
        clearLocalDBProgress();
        console.log("[Assessment] 🗑️ Cleared progress from LocalDB");
      }
    }
  };
          ...
          <Card className="border-2 shadow-2xl overflow-hidden">
            <CardContent className="p-4 sm:p-6 space-y-4">
              ...
              <div className="text-center space-y-2">
                <div className="mx-auto w-16 h-16 bg-linear-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center shadow-xl relative">
                    ...
                    ([category, score], index) => (
                      ...
                      >
                        <div className="flex justify-between items-baseline">
                          <span className="capitalize text-text font-bold text-sm">
                            {category}
                          </span>
                        ...
                        <div className="relative h-2 bg-background rounded-full overflow-hidden shadow-inner">
                          <div
                            className="h-full bg-linear-to-r from-primary-500 to-primary-600 rounded-full transition-all duration-1000 ease-out shadow-lg relative"
                            style={{
                              width: `${(score / 5) * 100}%`,
                          ...
                          >
                            <div className="absolute inset-0 bg-linear-to-r from-white/0 via-white/40 to-white/0 animate-[shimmer_2s_infinite]" />
                          </div>
                        </div>
                      </div>
                    )
              ...
              <div className="pt-2">
                <Button
                  ...
                  className="w-full bg-linear-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-bold py-3 text-base rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
                >
                  Retake Assessment
                </Button>
              </div>
      ...
      <Card className="shadow-xl shadow-primary-500/5 border-primary-500/10">
        <CardHeader className="pb-2">
          ...
          <div className="relative w-full h-1.5 bg-background rounded-full mb-2 overflow-hidden shadow-inner">
            <div
              className="absolute inset-0 bg-linear-to-r from-primary-500 via-primary-400 to-primary-500 rounded-full transition-all duration-500 ease-out shadow-lg shadow-primary-500/50"
              style={{
                width: `${progress}%`,
              ...
              <button
                ...
                className={`group relative w-full p-2.5 text-left rounded-lg border-2 transition-all duration-300 ${
                  selectedAnswer === index
                    ? "border-primary-500 bg-linear-to-br from-primary-50 to-primary-100/50 dark:from-primary-900/30 dark:to-primary-900/10 shadow-lg shadow-primary-500/20 scale-[1.01]"
                    : "border-border hover:border-primary-300 hover:shadow-md hover:scale-[1.005] bg-surface hover:bg-background"
                }`}
              >
                <span
                  className={`text-xs sm:text-sm transition-colors pr-7 ${
                    selectedAnswer === index
