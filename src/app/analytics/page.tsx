...
export default function AnalyticsPage() {
  ...
  const [hasUniversityAccess, setHasUniversityAccess] = useState<
    boolean | null
  >(null);
            ...
            <Card className="max-w-md w-full border-red-500/30 bg-red-500/5">
              <CardContent className=" text-center">
                ...
                <p className="text-text-muted mb-6">
                  This page is only accessible to university accounts. Contact
  ...
  const gapData =
    analytics.industryBenchmarks && analytics.enhanced?.skillsDetailed
      ? analytics.industryBenchmarks.topSkills
          .map((industry) => {
            const studentSkill = analytics.enhanced!.skillsDetailed!.find(
              (s) =>
                s?.name &&
                industry?.skill &&
                s.name.toLowerCase() === industry.skill.toLowerCase()
            );
            const studentCoverage = studentSkill
              ? studentSkill.percentageOfTotal
              : 0;
            return {
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="pt-16">
          <div className="max-w-7xl mx-auto p-6 md:p-8 space-y-8 pb-8">
              ...
              <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                ...
                <CardContent className="">
                  ...
                  <p className="text-3xl font-bold text-text">
                    {analytics.data.overview.totalStudents}
                  </p>
                ...
                <CardContent className="">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-semibold text-emerald-600">
                      {analytics.data.overview.avgProfileCompletion}%
                    </span>
                  </div>
                  <p className="text-3xl font-bold text-text">
                    {analytics.data.overview.avgProfileCompletion}%
                ...
                <CardContent className="">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-semibold text-purple-600">
                      {analytics.data.overview.totalCourses}
                    </span>
                  </div>
                  <p className="text-3xl font-bold text-text">
                    {analytics.data.overview.courseCompletionRate}%
                  </p>
                  <p className="text-sm text-text-muted mt-1">
                    Course Completion
                  </p>
                ...
                <CardContent className="">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-semibold text-primary-600">
                      {analytics.data.overview.totalPathways}
                    </span>
                  </div>
                  <p className="text-3xl font-bold text-text">
                    {analytics.data.overview.pathwayCompletionRate}%
                  </p>
                  <p className="text-sm text-text-muted mt-1">
                    Pathway Completion
                  </p>
            ...
            {/* Skills Gap */}
            {analytics.enhanced?.skillsDetailed &&
            ...
            {analytics.enhanced?.courseEngagement && (
              <CourseEngagementChart
                platformStats={
                  analytics.enhanced.courseEngagement.platformStats
                }
                difficultyStats={
                  analytics.enhanced.courseEngagement.difficultyStats
                }
              />
            )}
            ...
            <div className="text-center py-4">
              <p className="text-xs text-text-muted">
                Data updated {new Date(analytics.generatedAt).toLocaleString()}{" "}
                • All data anonymized
              </p>
            </div>
          </div>
